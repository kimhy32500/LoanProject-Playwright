# 신용대출 조회 테스트 자동화 프로젝트 (Playwright)

Playwright와 Pytest를 활용하여 직접 제작한 웹 대출 서비스의 주요 시나리오를 테스트 자동화한 프로젝트입니다.
...
> 테스트 자동화가 FAIL 상황을 정상적으로 감지하는지 확인하기 위한 케이스입니다.
...
이전 업무에서 이슈 리포팅 시 “어떤 화면에서 어떤 문제가 발생했는지”를 최대한 상세하게 전달하려고 노력했기 때문에, 테스트 자동화에서도 실패 상황을 바로 시각적으로 확인할 수 있도록 기능을 추가했습니다.  
...
반복 실행 시 플레이키 현상이 줄어드는 것을 경험하면서, 테스트 자동화에서 안정성이 얼마나 중요한지도 다시 느낄 수 있었습니다.
